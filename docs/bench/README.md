...
```bash
booktrans --bench-stats            # ветка X.Y встроенного ключа
...
где сломалась модель. Версия `1.4` берёт и `1.4.x`: третья цифра меняет
только формулировки, вердикты те же. Ловушки, которые проходят 95 % моделей
