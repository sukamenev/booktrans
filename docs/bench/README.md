...
Штраф, которого нет в ключе, в счёт не идёт.
...
точки не склеиваются; точку, которую проходят почти все модели
(`--bench-stats`), заменяют трудной. Полноту в целом держит штраф `OMIT`.
