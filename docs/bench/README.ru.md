...
по умолчанию — `codex:gpt-6-sol:medium`, запасной
`claude:claude-opus-5-5:medium`; другого называют ключом `--judge`. Текст
...
Версия бенчмарка — в ключе (`booktrans-bench-key 1.4.0`) и в отчёте, три
...
штрафы. Судья тоже не безошибочен: один и тот же судья на повторе того же
перевода расходится с собой в 4 точках из ста (Opus 5.5) или 7 (Sol 5.6), а
два разных судьи — в 10–16 точках из 107. Разница в два-три балла между
моделями поэтому ничего не значит, а баллы разных судей несравнимы: Opus
5.5 щедрее GPT-6 Sol в среднем на 8 баллов (от 3 до 15), потому что
пропускает кальки и смягчённый регистр, которые ключ запрещает; GPT-6 Sol
выбран по умолчанию как судья, точнее следующий ключу. Результаты замеров собираются в
...
`b0008-b0072`); точка на блок, которого нет в тексте, не упакуется. Общая
сумма очков не обязана быть круглой: итог нормируется к 100. Число точек ограничивает судья — весь
