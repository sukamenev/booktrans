...
def main():
    ...
    two = "- **Дензел Бел (Субер) (Densel Bel / Suber):** диверсант."
    ok("две скобки — куски по письменности",
       row(two, "NAMES")[0]
       == "| Densel Bel; Suber | Дензел Бел; Субер | | диверсант. |",
       row(two, "NAMES"))
    full = "| Bel (Suber) | Бел | | диверсант |"
    ok("скобки в ключе полной строки — часть написания",
       row(full, "NAMES") == (full, False, False), row(full, "NAMES"))
    ...
    ok("ключ первой ячейкой, перевод пуст",
    ...
    # Разведка до латинских ключей: русские заголовки, карточки с
    # подпунктами, род с прозвищем в скобках вместо оригинала.
    RUS = """## ПЕРСОНАЖИ

| Персонаж (Оригинал / Русский) | Кто это |
|---|---|
| **Квант (Quant)** | Ж. Пилот. |
| **Папа, Роум (Papa, Roam)** | Медведи. |
| **Маккоркл (McCorkle)** | М. Офицер. |
...
- **Роум (Roam)** — женский род.
- **Маккоркл (duo)** — мужской род.

## МИР: существа, вещи, места

### Существа

- **Разумные медведи (bears):** Гризли.
  - *Выбор слов:* `leg` → лапа.
...
- **Хинтерленд:** Город.
"""
    rus = R.canon_ref(RUS, "ru", legacy=True)[0]
    ok("русским заголовкам — латинский ключ",
       "## CHARACTERS — ПЕРСОНАЖИ" in rus and "## GENDER" not in rus, rus)
    ok("шапка со скобками выброшена", "Персонаж" not in rus, rus)
    ok("род по прозвищу в скобках — сущности по переводу",
       "| McCorkle | Маккоркл | мужской род. | М. Офицер. |" in rus, rus)
    ok("род в строке о нескольких именах — с именем",
       "| Papa, Roam | Папа, Роум | Роум: женский род. | Медведи. |" in rus,
       rus)
    ok("подпункт карточки — в её содержимое",
       "| bears | Разумные медведи | | Гризли; *Выбор слов:* `leg` → лапа. |"
       in rus, rus)
    ok("пустой подраздел выброшен, живой остался",
       "### Существа" not in rus and "### Места" in rus
       and "| Хинтерленд | Город. |" in rus, rus)
    ok("перекладка идемпотентна", R.canon_ref(rus, "ru") == (rus, 0, 1),
       R.canon_ref(rus, "ru")[1:])
