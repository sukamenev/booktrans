...
ANSWERS = {
    # Свободный ответ идёт в конверте: работа между маркерами, число в
    # них подставляет модель. Часть тут одна, оттого единица.
    "scout": "[[[SCOUT 1]]]\n## ИМЕНА\n\nПётр = Пётр\n[[[/SCOUT 1]]]",
...
def main():
    ...
    for name, text, want in (
            ("вызов инструмента", '[[[DIGEST 9]]]\nСейчас загляну.\n'
                                  '<invoke name="Read">\n</invoke>\n'
                                  'File does not exist.\n[[[/DIGEST 9]]]', False),
            ("ответ оболочки",
             f"[[[DIGEST 9]]]\n{good}\nFile does not exist.\n[[[/DIGEST 9]]]", False),
            ("обрывок", "[[[DIGEST 9]]]\nКоротко.\n[[[/DIGEST 9]]]", False),
            # Без конверта не принимается ничто: маркер с подставленным числом
            # в запросе не стоит, поэтому вернувшийся эхом промпт не пройдёт.
            ("без конверта", good, False),
            # Какое число подставлено, не сверяется: живая модель ошибалась
            # в нём, а повтор стоит целого запроса.
            ("конверт с другим числом",
             f"[[[DIGEST 8]]]\n{good}\n[[[/DIGEST 8]]]", True),
            ("маркеры с разными числами",
             f"[[[DIGEST 9]]]\n{good}\n[[[/DIGEST 8]]]", False),
            ("настоящий конспект", box, True)):
        try:
