...
from booktrans import lang as L                         # noqa: E402
...
COMPOUND = [
    ...
    # Дефис необязателен: «3D» и «2D» русский пишет словом — «трёхмерный».
    # На книге о восприятии таких мест было пятнадцать из двадцати шести
    # ложных сообщений раздела.
    ("The 3D interpretation of the parts", "3", True),
    ("rather than a 2D flat line-drawing", "2", True),
    # Строчная буква не в счёт: «5km» — мера, и цифра там обязана уцелеть,
    # а «H2O» — формула, где цифра стоит между букв.
    ("after 5km of road", "5", False),
    ("the H2O molecule", "2", False),
]
...
# (перевод, число оригинала, целевой язык, написано ли оно словом).
# «Turn the picture 45 degrees» по-русски пишется «на сорок пять градусов»:
# цифра исчезает по правилам языка, а не по недосмотру.
WORDNUM = [
    ("повернуть на сорок пять градусов", "45", "ru", True),
    ("до девяноста процентов времени", "90", "ru", True),
    ("показаны на сто миллисекунд", "100", "ru", True),
    ("пучок из двухсот миллионов волокон", "200", "ru", True),
    ("более тысячи слов", "1000", "ru", True),
    # Существительное от цифры — тоже она: «есть ли четвёрка в фигуре HB».
    ("в фигуре HB есть четвёрка", "4", "ru", True),
    # Нужны все разряды: одного «сорок» для сорока пяти мало.
    ("сорок человек в зале", "45", "ru", False),
    # Ложные друзья: «однако» — не «один», «двери» — не «два».
...
MEASURE = [
    ...
    # Мили в час: «100 mph» → «160 км/ч». Меры не было в списке, и проверка
    # ругалась дважды — на пропавшую сотню и на взявшиеся ниоткуда сто
    # шестьдесят.
    ("thundering down at more than 100 mph", "летящую свыше 160 км/ч", "100",
     False, True),
    ("thundering down at more than 100 mph", "летящую свыше 160 км/ч", "160",
     True, True),
...
def main():
    ...
    for text, n, want in COMPOUND:
        ...
        print(f"    {'вычтено ' if got else 'засчитано'}  {'совпадает' if ok else 'РАСХОЖДЕНИЕ'}"
              f"  [{n}] {text[:52]}")
        bad += not ok
    print("  число, написанное словом:")
    for text, num, to, want in WORDNUM:
        got = L.spelled_out(text, num, to)
        ok = got == want
        print(f"    {'вычтено ' if got else 'засчитано'}  {'совпадает' if ok else 'РАСХОЖДЕНИЕ'}"
              f"  [{num}] {text[:52]}")
        bad += not ok
