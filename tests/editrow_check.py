...
цепочка редакторов идёт как задана, пропуская записанных отказавшихся, а
судьбу переведшей модели решает ключ --self-edit: allow — очередь как
задана, last — переведшая в конец очереди, never — вычёркивается совсем.
Отказавшиеся должны быть записаны в файл куска по именам — булева пометка
не говорит, кого обходить.
...
def main():
    ...
    # По умолчанию (--self-edit allow) очередь как задана.
    row = P._edit_row(opus, [pro, sol], "flash", False)
    ok("allow: цепочка как задана", names(row) == ["opus", "pro", "sol"],
       names(row))
    row = P._edit_row(opus, [pro, sol], "opus", False)
    ok("allow: самоправка не мешает очереди",
       names(row) == ["opus", "pro", "sol"], names(row))

    # last: переведшая модель — в конец очереди, даже главный редактор.
    row = P._edit_row(opus, [pro, sol], "opus", False, "last")
    ok("last: переведший главный редактор — в конец",
       names(row) == ["pro", "sol", "opus"], names(row))
    row = P._edit_row(opus, [pro, sol], "pro", ["flash"], "last")
    ok("last: переведшая дном и при именах",
       names(row) == ["opus", "sol", "pro"], names(row))
    row = P._edit_row(opus, [pro, sol], "sol", False, "last")
    ok("last: переводчик уже дно — порядок цел",
       names(row) == ["opus", "pro", "sol"], names(row))

    # true: переведшая вычёркивается совсем; пустая очередь — кусок без правки.
    row = P._edit_row(opus, [pro, sol], "opus", ["flash"], "never")
    ok("never: переведшая вычеркнута", names(row) == ["pro", "sol"],
       names(row))
    row = P._edit_row(opus, [pro, sol], "pro", None, "never")
    ok("never: старый файл — запрет главнее",
       names(row) == ["opus", "sol"], names(row))
    row = P._edit_row(opus, [pro, sol], "pro", ["opus", "sol"], "never")
    ok("never: остался один переводчик — очередь пуста", names(row) == [],
       names(row))
    ...
    # Имена записаны: цепочка за вычетом отказавшихся, порядок как задан.
    row = P._edit_row(opus, [pro, sol], "pro", ["flash"])
    ok("редактор чист — цепочка как задана",
       names(row) == ["opus", "pro", "sol"], names(row))
    ...
    # Переведшей модели нет в цепочке редакторов — идут чистые.
    row = P._edit_row(opus, [sol], "pro", ["flash"])
    ok("переведшей нет в цепочке — идут чистые", names(row) == ["opus", "sol"],
       names(row))

    # Отказали все редакторы: остаётся переведшая — дно, а не первый выбор.
