...
переведшая модель всегда встаёт в конец очереди — даже когда она главный
редактор. Для этого отказавшиеся должны быть записаны в файл куска по
именам — булева пометка не говорит, кого обходить.
...
def main():
    ...
    # Обычный кусок: цепочка как задана, переводчика в ней нет.
    row = P._edit_row(opus, [pro, sol], "flash", False)
    ok("без отказа — цепочка как задана", names(row) == ["opus", "pro", "sol"],
       names(row))

    # Обычный кусок, переведённый главным редактором: он уходит в конец.
    row = P._edit_row(opus, [pro, sol], "opus", False)
    ok("переведший главный редактор — в конец очереди",
       names(row) == ["pro", "sol", "opus"], names(row))

    # Переводчик и так дно очереди — порядок не меняется.
    row = P._edit_row(opus, [pro, sol], "sol", False)
    ok("переводчик уже дно — порядок цел",
    ...
    # Отказника нет в цепочке, а перевёл главный редактор: правит второй.
    row = P._edit_row(opus, [pro, sol], "opus", ["flash"])
    ok("главный редактор перевёл — правит следующий",
       names(row) == ["pro", "sol", "opus"], names(row))

    # Первый редактор отказался переводить — пропущен, правит второй.
    row = P._edit_row(opus, [pro, sol], "pro", ["opus"])
    ok("отказавшийся редактор пропущен, переведшая дном",
       names(row) == ["sol", "pro"], names(row))
