...
def main():
    ...
    # Служебный отчёт, дописанный после закрывающего маркера, отброшен молча;
    # без закрывающего маркера исправление не принимается; идентификатор
    # блока внутри текста для книги отвергается на любом языке.
    v, n, f = P._parse_verify(
        OUT.replace("[[[/P s39.b0011]]]",
                    "[[[/P s39.b0011]]]\n\nОстальные абзацы расхождений"
                    " с оригиналом не дают."), WANT)
    ok("дописанное после маркера отброшено",
       f["s39.b0011"] == "…сотая часть массы Млечного Пути…", f)
    try:
        P._parse_verify(OUT.replace("\n[[[/P s39.b0011]]]", ""), WANT)
        ok("исправление без закрывающего маркера отвергается", False)
    except ValueError as e:
        ok("исправление без закрывающего маркера отвергается",
           "не закрыто" in str(e), e)
    try:
        P._parse_verify(
            OUT.replace("…сотая часть массы Млечного Пути…",
                        "…сотая часть массы (см. b0085) Млечного Пути…"), WANT)
        ok("идентификатор блока в тексте отвергается", False)
    except ValueError as e:
        ok("идентификатор блока в тексте отвергается", "служебн" in str(e), e)
    # Отчёт без идентификаторов, влезший между маркерами, выдаёт себя длиной:
    # исправление не бывает вдвое длиннее оригинала.
    size = {"s39.b0011": 40}
    try:
        P._parse_verify(
            OUT.replace("…сотая часть массы Млечного Пути…",
                        "…сотая часть массы Млечного Пути…\n\n"
                        + "Прилипший к абзацу разговор о работе. " * 6),
            WANT, size=size)
        ok("разросшееся исправление отвергается", False)
    except ValueError as e:
        ok("разросшееся исправление отвергается", "длиннее" in str(e), e)
    v, n, f = P._parse_verify(OUT, WANT, size=size)
    ok("обычное исправление проходит при мерке длины",
       f == {"s39.b0011": "…сотая часть массы Млечного Пути…"}, f)
    # Промах идентификатором: под своим номером приходит копия соседнего
    # абзаца, непохожая на прежний текст своего, — отвергается.
    twin_text = "Насекомые начали осыпаться, теряя хватку и способность держаться в воздухе, а я двинулась вперёд."
    snap = {"s39.b0011": "Сотая часть массы, о которой шла речь в докладе.",
            "s22.b0041": twin_text}
    try:
        P._parse_verify(
            OUT.replace("…сотая часть массы Млечного Пути…", twin_text),
            WANT, snap=snap)
        ok("копия соседнего абзаца отвергается", False)
    except ValueError as e:
        ok("копия соседнего абзаца отвергается", "повторяет" in str(e), e)
    v, n, f = P._parse_verify(OUT, WANT, snap=snap)
    ok("своё исправление при снимке проходит",
       f == {"s39.b0011": "…сотая часть массы Млечного Пути…"}, f)
    try:
