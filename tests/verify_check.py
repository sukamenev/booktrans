...
def main():
    ...
    # Служебный отчёт, приклеенный к исправлению или сноске, уезжал прямиком
    # в книгу — такой ответ отвергается целиком, и повтор несёт причину.
    try:
        P._parse_verify(
            OUT.replace("…сотая часть массы Млечного Пути…",
                        "…сотая часть массы Млечного Пути…\n\nОстальные абзацы"
                        " расхождений с оригиналом не дают."), WANT)
        ok("отчёт внутри исправления отвергается", False)
    except ValueError as e:
        ok("отчёт внутри исправления отвергается", "служебный" in str(e), e)
    try:
