...
"""
    ...
    # Бюджет: на сиквеле совпадает полтысячи строк, и запрос перерастает
    # обрез транспорта. При переполнении первыми выживают чаще упомянутые,
    # порядок уцелевших — прежний; без бюджета отбор не меняется.
    rows = [("Rare / Рар", "|строка про Rare|"),
            ("Dense / Денс", "|строка про Dense|"),
            ("Mid / Мид", "|строка про Mid|")]
    text = "Dense и Mid спорят. Dense зовёт Mid, Dense смеётся, Rare молчит."
    ok("без бюджета совпадают все",
       P.ref_rows_for(rows, text) == [r[1] for r in rows], None)
    cut = P.ref_rows_for(rows, text, budget=40)
    ok("бюджет держит чаще упомянутые, порядок прежний",
    ...
    aframe, arows = P.split_ref("## CHARACTERS — Персонажи\n\n"
                                "| Оригинал | Перевод |\n|---|---|\n"
                                "| Dense | Денс |\n| Rare; Rar | Рар |\n\n"
                                "## ADDRESS — Обращения\n\n"
                                "Умолчание: сокомандники на «ты».\n\n"
                                "| Кто → кому | Форма |\n|---|---|\n"
                                "| Dense; Mid | ты |\n| Dense; Ghost | вы |\n")
    ok("раздел едет со строкой, шапка обращений — нет", [r[2] for r in arows]
       == ["CHARACTERS", "CHARACTERS", "ADDRESS", "ADDRESS"], arows)
    ok("умолчание обращений — в костяке",
       aframe.strip() == "## ADDRESS — Обращения\n\nУмолчание: сокомандники на «ты».",
       aframe)
