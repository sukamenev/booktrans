...
def main():
    ...
    # Справочник без конверта не принимается, даже когда похож на работу:
    # закрывающий маркер — единственное доказательство, что ответ дошёл до
    # конца, и обрубок с парой разделов сошёл бы за целый справочник.
    try:
        P._parse_scout(f"Сейчас соберу.\n{body}")
        got_unboxed = "приняли"
    except ValueError:
        got_unboxed = ""
    ok("справочник без конверта отвергнут", not got_unboxed, got_unboxed)
    ...
    # Сведение частей записало перемену «было → стало» и в выходные данные:
    # книга вышла с заглавием из трёх названий и номеров частей.
    open(f"{d}/xx/scout.md", "w", encoding="utf-8").write(
        "## META — Выходные данные\n\ntitle_target = Страж → Падение (ч. 78)\n"
        "genre = sf → sf_heroic (ч. 76)\n")
    got = P.scout_meta(d, "xx")
    ok("перемена в выходных данных — берётся первое",
       got.get("title_target") == "Страж" and got.get("genre") == "sf", got)

    # Фамилию называет разведка, и принимается она, только если стоит в имени
    # автора слово в слово: чужое слово имени не разделит.
    for sn, want in (("Гарсиа Маркес", "Гарсиа Маркес"), ("Борхес", None)):
        open(f"{d}/xx/scout.md", "w", encoding="utf-8").write(
            "## META — Выходные данные\n\nauthor = Gabriel García Márquez\n"
            f"author_target = Габриэль Гарсиа Маркес\nauthor_surname = {sn}\n")
        got = P.scout_meta(d, "xx")
        ok(f"фамилия от разведки: {sn}", got.get("author_surname") == want, got)
    ...
    ok("в опасных местах не ищем: там разные значения по замыслу",
       P._forked(f"## RISK — Опасные места\n\n{pair}", "ru") == [])
    ok("кандидат в сноску с двумя переводами — развилка",
       len(P._forked("## FOOTNOTES — Сноски\n\n| void fraction | доля пустот / "
                     "пористость | физический показатель |", "ru")) == 1)
    ok("точка с запятой — тоже развилка",
       len(P._forked("## TERMS — Термины\n\n| proxy | марионетка; подконтрольный "
                     "| | человек под контролем |", "ru")) == 1
       and P._forked("## CHARACTERS — Персонажи\n\n| Victoria; Antares | Виктория; "
                     "Антарес | ж | героиня |", "ru") == [])
    ...
    # такие строки переспрашивались каждый прогон и пугали предупреждением.
    quoted = ("| Оригинал | Русский |\n|---|---|\n"
              "| kiss-kill | «поцелуй или убей» |\n")
    ok("закавыченная фраза с «или» — не развилка",
       P._forked(f"## TERMS\n\n{quoted}", "ru") == [])
    vs = ("| Оригинал | Русский |\n|---|---|\n"
    ...
    # Двойники персонажей: один герой под двумя ключами — обе карточки
    # ехали в каждый его кусок. Совпавший псевдоним сливает группы.
    f_c = ("## CHARACTERS — Персонажи\n\n"
           "| Рой; Тейлор (Skitter; Taylor) | школьница |\n")
    f_d = ("## CHARACTERS — Персонажи\n\n"
           "| Рой; Тейлор Эберт (Skitter; Taylor Hebert) | взрослеет |\n"
           "| Мрак (Grue) | руководитель |\n")
    o2, g2, h2 = P._registry([f_c, f_d])
    twin = [(gk, vs) for (gk, _, _), vs in
            ((x, g2[x[0]]) for x in o2) if len(vs) > 1]
    ok("двойники слиты в одну группу",
       len(twin) == 1 and len(twin[0][1]) == 2
       and sum(1 for x, _, _ in o2 if g2[x]) == 2,
       [(gk[1], len(vs)) for gk, vs in twin])
    ok("чужой персонаж не прилип",
       any("Мрак" in v for _, v in g2[[x for x, _, _ in o2 if "grue" in x[1]][0]]),
       None)

    # Тёзки не сливаются: совпавшее переводное имя-одиночка («Сара») бывает
    # у разных людей; двойника выдаёт только общий оригинальный псевдоним.
    f_e = ("## CHARACTERS — Персонажи\n\n"
           "| Лиза; Сара; Сплетница (Lisa; Tattletale) | аналитик |\n"
           "| Сара (Sarah) | сестра Кэрол, позднее Леди Фотон |\n")
    o3, g3, h3 = P._registry([f_e])
    ok("тёзки остались разными персонажами",
       sum(1 for x, _, _ in o3 if g3[x]) == 2,
       [(x[1], len(g3[x])) for x, _, _ in o3])
