...
def main():
    ...
    # Оформление строки её не меняет: модель, ужимая пояснение, ставит имя
    # полужирным — и охранник считал бы строку потерянной, хотя она на месте.
    ok("выделение и регистр ключа не меняют",
       P._rows("| **Отец** | Актёр | долгое пояснение |")
       == P._rows("| отец | Актёр |"), P._rows("| **Отец** | Актёр |"))

    # Модель, вычеркнувшая таблицу, отвергается — и справочник остаётся как
    # был. Полтора предела лучше, чем книга без половины имён. В разделе без
    # таблиц стеречь нечего, и там мерой служит длина: ответ короче половины
    # запрошенного — это не сжатие, а выброшенный раздел.
