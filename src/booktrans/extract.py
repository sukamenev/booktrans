...
def _pdf_visual(path, agent, marks=None):
    ...
    for b in blocks:
        ...
        # Разметка внутри строки — общим разбором с чтением markdown. Своя
        # регулярка тут не понимала экранирования, и разделитель сцен
        # `\* \* \*` превращался в `\<i> \</i> \*`: в epub читатель видел
        # `\ \ \*`. Модель такое переводила как есть, дословно.
        # Строка из звёздочек или тире — разделитель сцен, а не абзац. Так
        # его и рисует каждый сборщик, и в модель он не идёт вовсе. Косые
        # снимаем перед проверкой: в markdown разделитель пишут и так, и так.
        if b["kind"] == "p" and MD_RULE.match(b["text"].replace("\\", "").strip()):
            b["kind"], b["text"] = "break", ""
            continue

        links = b.get("links") or []
        b["text"] = _md_spans(b["text"], links)
        if links:
            b["links"] = links
