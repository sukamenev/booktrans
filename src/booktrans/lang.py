...
NAME_TRIM = {
    "ru": r"[аеёиоуыэюяйь]{1,2}$", "uk": r"[аеєиіїоуюяйь]{1,2}$",
    "be": r"[аеёіоуыэюяйь]{1,2}$", "pl": r"[aąeęioóuy]{1,2}$",
...
def name_trim(word, to):
    ...
    короче трёх букв не режем: слово целиком."""
    code = (to or "")[:2]
    pat = NAME_TRIM.get(code)
    cut = re.sub(pat, "", word) if pat else word
    if len(cut) < 3:
        return [word]
    out = [cut]
