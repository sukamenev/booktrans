...
# Окончания имён по языкам: что отрезать, чтобы найти имя в любой форме.
# Языка нет в таблице — имя ищется целиком; чередование основы («Рот —
# Рта») в любом языке закрывают формы, названные разведкой.
NAME_TRIM = {
    "ru": r"[аеёиоуыэюяйь]$", "uk": r"[аеєиіїоуюяйь]$",
    "be": r"[аеёіоуыэюяйь]$", "pl": r"[aąeęioóuy]$",
    "de": r"(?:es|en|s|n)$", "en": r"(?:[’']s|s)$",
}
# Сколько раз снимать окончание: гласные — по одной, до двух («Лощина» →
# «Лощин», «Ополчение» → «Ополчен»), суффиксы de/en — один раз.
NAME_TRIM_STEPS = {"ru": 2, "uk": 2, "be": 2, "pl": 2}
# Беглая гласная перед последним согласным: «Чертёнок — Чертёнка», «Ясновидец
# — Ясновидца». У основы появляется второй вид, без гласной.
NAME_FLEET = {"ru": r"[еёо](?=[кцнл]$)", "uk": r"[еоі](?=[кцнл]$)", "be": r"[еёо](?=[кцнл]$)"}


def name_trim(word, to):
    """Основы имени по правилу языка — одна или две (с беглой гласной);
    режем по букве, пока основа не короче трёх: «Змей» → «Зме»."""
    code = (to or "")[:2]
    pat = NAME_TRIM.get(code)
    cut = word
    for _ in range(NAME_TRIM_STEPS.get(code, 1) if pat else 0):
        nxt = re.sub(pat, "", cut)
        if nxt == cut or len(nxt) < 3:
            break
        cut = nxt
    out = [cut]
    fleet = NAME_FLEET.get(code)
    if fleet and re.search(fleet, cut):
        out.append(re.sub(fleet, "", cut))
    return out
