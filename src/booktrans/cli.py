...
_blank = [False]
# Строка не закрыта: прошлый вызов печатал с end="" — продолжение той же
# строки штамповать нельзя.
_mid = [False]
...
def main():
    ...
    if os.path.exists(bp):
        ...
        if os.path.splitext(args.book)[1].lower() in (".pdf", ".txt", ".md") \
                and not extract.is_markdown(args.book):
    ...
    # всей книги: перевод лежит рядом, но под прежними номерами. Перепривязка
    # идёт по отпечаткам и обязана случиться раньше перевода и сборки.
    pipeline.reanchor(work, blocks, args.to, log)
