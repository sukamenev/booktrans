...
def judges(args, models, translator):
    """Цепочка судей. Судья одной семьи с переводчиком допущен по умолчанию:
    на проверке 1.4.0 самосуд себе не льстил (GPT-6 Sol поставил себе меньше,
    чем ему дал Opus), а вычёркивать его — значит отдавать целую семью
    моделей запасному судье с другой шкалой; один судья на всех важнее.
    `--self-edit never` возвращает запрет, `last` ставит родню в конец."""
    ...
    mode = getattr(args, "self_edit", None) or "allow"
    out, kin, dropped = [], [], []
