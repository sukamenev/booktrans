...
# Частицы фамилии: «ван Гог», «Ле Гуин», «де Сент-Экзюпери», «фон Браун».
NAME_PARTICLES = {"de", "del", "della", "di", "da", "das", "dos", "du", "la", "le", "van",
                  "von", "der", "den", "ter", "ten", "bin", "ibn", "al", "el", "st.",
                  "де", "дель", "делла", "ди", "да", "дас", "дос", "дю", "ла", "ле", "ван",
...
def author_parts(raw):
    ...
    Фамилия — последнее слово вместе с частицами перед ним: «Эдгар Аллан По»,
    «Эрих Мария Ремарк», «А. Виктор Хоффбранд», «Урсула Ле Гуин». Прежде
    ...
    while cut > 1 and au[cut - 1].lower() in NAME_PARTICLES:
        cut -= 1
    return " ".join(au[:cut]), " ".join(au[cut:]), alias
...
NOTE_MARK_WS = re.compile(r"\s?\[\^\d+\]")


def strip_note_marks(text):
    """Убрать служебные указатели сносок из текста без сноски: их термин
    уже объяснён раньше, и сборка сноску не ставит."""
    return NOTE_MARK_WS.sub("", text)


def anchor_note(text, bid, terms):
    """Вставить метку сноски по указателю переводчика или после термина.

    Первый эшелон — служебный указатель `[^N]`: переводчик ставит его сразу
    за поясняемым словом, редактура обязана сохранить (конвейер проверяет),
    и знак встаёт ровно туда. Указатель принимает знак один — сноски блока
    сборка сливает в одну; остальные вычищаются.

    Второй эшелон — поиск термина: переводы без указателей (старые книги,
    сноски отдельного прохода) якорятся по слову из TERM. Знак сноски стоял
    в конце абзаца, каким бы длинным тот ни был: читатель видел номер за
    сотню слов от объясняемого оборота и гадал, к чему он. Термин в переводе
    бывает склонён, поэтому каждому слову позволен хвост до трёх букв. Не
    нашлось ни одного — вернуть текст как есть: знак в конце абзаца честнее
    знака не у того слова.
    """
    m = NOTE_MARK.search(text)
    if m:
        head = text[:m.start()].rstrip()
        return head + NOTE_AT.format(bid) + strip_note_marks(text[m.end():])
    # Модель порой пишет в TERM несколько слов разом — «микроже, же» — и
    # целиком такая строка в тексте не встречается. Разбираем по запятым и
    # косым; куски короче трёх знаков не ищем: «же» нашлось бы в любой фразе.
    parts = []
    for t in terms or ():
        t = re.sub(r"<[^>]+>", "", str(t)).strip()
        parts += [x.strip() for x in re.split(r"[,;/]", t)]
    parts = [x for x in dict.fromkeys(parts) if len(x) >= 3]
...
def is_math(s):
    ...
    if not s or len(s) > MATH_MAX:
        return False
    # Деньги, а не формула: вилка цен «$5-$10» (между долларами «5-», оборванное
    # на знаке) и сумма с разрядами «$1,000,000$».
    if re.fullmatch(r"\d[^\\^_{}]*[-/,:;+=(]", s) or re.fullmatch(r"\d{1,3}(?:[,.\s]\d{3})+(?:\.\d+)?", s):
        return False
    ...
    # Слитно и не обрываясь на знаке: «$13q14$», «$0,54$», «$(ETV6::RUNX1)$».
    if re.fullmatch(r"[\w(][^\s$]*[\w)]|\w", s):
        return True
