...
# Перенос строки внутри абзаца: модель пишет его и как <br>, и как <br/>.
BR = re.compile(r"<br\s*/?>", re.I)


def strip(s, sep=""):
    return TAG.sub(sep, BR.sub(" ", s))
...
def fingerprint(t):
    ...
    """
    # Перенос строки — часть текста: книга, перечитанная с переносами,
    # должна переперевести именно эти блоки.
    return hashlib.sha1(
        re.sub(r"\s+", " ", strip(BR.sub(" ⏎ ", t))).strip().encode("utf-8")).hexdigest()[:12]
...
_HEADER_KEYS = {"оригинал", "original", "в оригинале", "имя", "name",
                "имя в оригинале", "персонаж", "character", "персонажи",
                "characters", "сущность", "объект", "термин", "term", "слово",
                "word", "перевод", "translation", "ключ", "key", "кто", "кому",
                "who", "whom"}
...
def scout_meta(work, to=""):
    ...
    for line in head.splitlines():
        ...
        # Сведение частей пишет перемену «было → стало»; выходные данные по
        # сюжету не меняются — берём первое.
