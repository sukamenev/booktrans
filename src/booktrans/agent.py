...
class OutputLimit(AgentError):
    """Ответ упёрся в предел вывода: модель зациклилась или разлилась. Это
    изъян самой модели, а не связи — повторять можно, винить роутер нельзя."""
...
def reset_after(msg):
    ...
    # Срок, минувший перед самой полуночью, после неё выглядит как «завтра
    # без десяти минут» — это тот же свежепрошедший срок.
    if -600 < left <= 0 or left > 86400 - 600:
        return 120
...
def collect_stream(events, model):
    ...
    if finish == "length":
        ...
        raise OutputLimit(T("oa_length", (tok or {}).get("out", "?")))
    if not text and (tok or {}).get("out"):
        # Вывод потрачен, а ответа нет: всё ушло в рассуждения, и точка
        # срезала их своим потолком, назвав это обычной остановкой (byNara
        # режет на 32768 токенах при запрошенных 65536 и пишет «stop»).
        raise OutputLimit(T("oa_empty", tok["out"]))
    if text and finish is None:
        # Поток кончился без причины остановки — роутер уронил соединение на
        # полуслове. Иначе обрывок уходил в разбор, и выходило «модель встала
        # на блоке N» с отказом по содержанию вместо ожидания и повтора.
        raise AgentError(T("oa_cut", len("".join(text))))
